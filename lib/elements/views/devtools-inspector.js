...
export class DevToolsInspector extends LitElement {
    ...
    _initSubChannel() {
        ...
        document.addEventListener(MESSAGE_TYPE.SELECT_RESULT.toString(), (/** @type {CustomEvent} */ event) => {
            ...
            this.selectedElementParseError = this.selectedElement == null;
        });
    }

    firstUpdated() {
        this.customElementsList = document.querySelector('devtools-elements-list');

        document.addEventListener("devtools-element-selected", e => this.setSelectedElement(e.detail.elem))
    }

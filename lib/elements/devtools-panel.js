...
import { MESSAGE_TYPE, QueryMessage, SelectResultMessage } from '../types/message-types.js';
...
export class DevToolsPanel extends LitElement {
    ...
    firstUpdated() {
        ...
        document.addEventListener(new QueryMessage().type, (/** @type {CustomEvent} */ event) => {
            console.log("ON QUERY RESULT");
            this.loading = false;
            this.queryResult = event.detail;
        });
        document.addEventListener(new SelectResultMessage().type, (/** @type {CustomEvent} */ event) => {

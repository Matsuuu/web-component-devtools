...
export class CustomElementsInspector extends LitElement {
    ...
    updatePropertyValue(key, value) {
        ...
        this._logObject("Update property value: ", { key, value, elementType });
    ...
    onResizeMouseMove(e) {
        // TODO(Matsuuu): Make this work with bottom devtools too
        const newFlexPercentage = (
            100 -
    ...
    render() {
      ...
      ${this.showHeader()} ${this.showProperties()}
    `;
    }
    ...
    renderObjectProperty(prop, propName) {
      ...
      <li>
        <textarea
          @input=${(e) =>
                this.updateJSONPropertyValue(prop.key, e.target.value)}
        >
${prop.value ? JSON.stringify(prop.value, null, 2) : ""}</textarea
    ...
    renderNumberProperty(prop, propName) {
      ...
      <li>
        <input
          @input=${(e) => this.updatePropertyValue(prop.key, e.target.value)}
          type="number"
          value=${prop.value ?? ""}
        />
      </li>
...
${prop.value ?? ""}</textarea
    ...
    renderBooleanProperty(prop, propName) {
      ...
      <li>
        <input
          @input=${(e) => this.updatePropertyValue(prop.key, e.target.checked)}
          type="checkbox"
          ?checked=${!!prop.value}
        />
      </li>

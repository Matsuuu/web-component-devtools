...
export class DevToolsListItem extends LitElement {
    ...
    firstUpdated() {
        if (this.isUndefined) {
            this.title =
                "This element has not been defined in the customElements registry";
            return;
        }
    ...
    updated(_changedProperties) {
        if (_changedProperties.has("isUndefined") &&
            _changedProperties.get("isUndefined") === true && !this.isUndefined) {
            this.hydrated = true;
            this._initializeEventListeners();
        }
    }
    ...
    _spotlight() {
        postMessage({
            ...
            id: this.element.index,
        });
    }

    _spotlightOff() { postMessage({ type: MESSAGE_TYPE.HIGHLIGHT, index: -1 }); }
            ...
            <li style="padding-left: ${this.relativeDepth}rem">
                <span class="list-item-flexer">
                    <span class="custom-element-span">
                        ${this.hasChildren ? html` <span class="child-toggler" @click=${this._toggleChildren}>${ARROW_UP}</span> `
                : ''}
                        <span>${this.element.name}</span>
                        ${this.isUndefined
                ? html`<span class="info-text">Element not defined</span>`
                : ''}
                ${this.hydrated ? html`<span title="This element was hydrated">💧</span>`
                : ''}
                    </span>
                    ${this.renderHtmlTag()}
                </span>
    ...
    renderHtmlTag() {
        ...
        return html`
            <span class="html-tag"> ${this.selected ? nodeString
                : nodeString?.substring(0, limit) + ellipsis}</span>
        `;
    }

...
class CustomElementList extends LitElement {
    ...
    _spotlight(elem) {
        ...
        this._highlightChildrenInTree(elem);
    }

    _spotlightOff() {
        postMessage({ type: MESSAGE_TYPE.HIGHLIGHT, index: -1 });
        this._unHighlightChildrenInTree();

        if (this.selectedElement) {
            this._spotlight(this.selectedElement);
        }
    }
    ...
    _highlightChildrenInTree(elem) {
        ...
        children.forEach((child) => child.setAttribute("child-of-hovered", ""));
    }

    _unHighlightChildrenInTree() {
        this.shadowRoot
            .querySelectorAll("[child-of-hovered]")
            .forEach((el) => el.removeAttribute("child-of-hovered"));
    }

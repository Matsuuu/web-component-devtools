 ...
 * @param {string} tagName
    *
 * @returns {import('custom-elements-manifest/schema').JavaScriptModule}
 */
export function findModuleByCustomElementTagName(manifest, tagName) {
    return manifest.modules.find(mod => hasCustomElementDefinition(mod) && mod.exports.some(ex => ex.name === tagName));
}
...
export function getElementMethods(customElement) {
    return /** @type { Array<import('custom-elements-manifest/schema').ClassMethod> } */ (
        customElement.members?.filter(isMethodMember)
    );
}

...
export function getPackageJsonUrl() {
    return getBaseUrl() + '/package.json';
}
...
export function getCustomElementsManifestUrl() {
    ...
    if (metaTag) return buildCustomElementsManifestPath(metaTag.content);

    return buildCustomElementsManifestPath('custom-elements.json');
}

/**
 * @param {string} path
 */
function buildCustomElementsManifestPath(path) {
    if (path.includes('://')) return path; // e.g. https://my-site.com/custom-elements.json
    if (path.startsWith('/')) return getBaseUrl() + path; // e.g. /dist/custom-elements.json
    return getBaseUrl() + '/' + path; // e.g. custom-elements.json
}

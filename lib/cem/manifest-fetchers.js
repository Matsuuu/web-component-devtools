 ...
 * */
export async function fetchManifest(baseUrl, customElementsPath) {
    let manifestUrl;
    if (isDevMode()) {
        const packageJsonUrl = getPackageJsonUrl(baseUrl);
        PACKAGE_JSON = await fetchPackageJson(packageJsonUrl);
        customElementsPath = customElementsPath ?? (PACKAGE_JSON ? PACKAGE_JSON.customElements : '');
        ...
        manifestUrl = getCustomElementsManifestUrl(baseUrl, customElementsPath);
    }

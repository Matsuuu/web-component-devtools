...
function getParentManifest(manifestData, elementData) {
    return Object.values(manifestData).find(
        dec => dec.name === elementData.parentClass?.name,
    );
}
...
export function mergeManifestDatas(targetData, newData) {
    targetData.attributes = [
        ...(targetData.attributes ?? []),
        ...(newData.attributes?.map((/** @type {any} */ attr) => addInheritanceIfMissing(attr, targetData.parentClass)) ?? []),
    ];
    targetData.properties = [
        ...(targetData.properties ?? []),
        ...(newData.properties?.map((/** @type {any} */ prop) => addInheritanceIfMissing(prop, targetData.parentClass)) ?? []),
    ];
    targetData.events = [
        ...(targetData.events ?? []),

...
let analyzedScripts = {};
let fetchedSources = {};
let fullPageManifest;

export function reset() {
    fullPageManifest = null;
    analyzedScripts = {};
    fetchedSources = {};
}
...
async function analyzeAllScripts() {
    if (fullPageManifest) return fullPageManifest;

    let manifest = await createManifest('', []);
    manifest.modules = [];
    for (const scrip of Array.from(document.querySelectorAll('script'))) {
        let modules;
        if (scrip.src) {
            const scriptSource = await fetchSource(scrip.src);
            modules = await analyzeScript(scrip.src, scriptSource);
        } else {
            modules = await analyzeScript(window.location.origin, scrip.innerText);
        }
        if (getDeclarationAndExportCount(modules) > 0) {
            manifest.modules = [...manifest.modules, ...modules];
        }
    }
    fullPageManifest = manifest;
    return fullPageManifest;
}
 ...
 */
export async function analyzeAndUpdateElement(elementData) {
    //if (!fullPageManifest) {
    fullPageManifest = await analyzeAllScripts();
    //}

    let elementModule = findModuleByCustomElementTagName(fullPageManifest, elementData.tagName);

    if (!elementModule) {
        // If we can't find the definition of the element, get what we can and exit eary
        await updateElementDataViaAnalysis(elementData);
        return;
    }
 ...
 * @param {DevToolsElement} elementData
 */
async function updateElementDataViaAnalysis(elementData) {
    // If we can't find the definition of the element, get what we can and exit eary
    const elementManifest = await createManifest(
        elementData.declaration,
        getNeededPlugins(elementData),

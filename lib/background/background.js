...
function bridgeToPanel(message) {
    if (message.type === MESSAGE_TYPE.MANIFEST_FETCH) {
        loadCustomElementManifestData(message.baseUrl, message.customElementsPath);
        return;
    }
...
async function loadCustomElementManifestData(baseUrl, customElementsPath) {
    const customElementManifestData = await mapCustomElementManifestData(baseUrl, customElementsPath);
    nydus.message(CONNECTION_CHANNELS.DEVTOOLS_BACKGROUND_CONTENT, {
...
async function analyzeCustomElement(message) {
    ...
    await analyzeAllScripts(message.pageSources, message.tabId);

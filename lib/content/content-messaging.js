...
function doSelect(message) {
    const selectResponse = waitForDomMessageResponse(SELECT_RESULT);
    const eventData = { index: message.index };
    if (customElementManifestData[message.name]) {
        eventData.manifestData = customElementManifestData[message.name];
    }
    document.dispatchEvent(
    ...
    selectResponse.then(res => {
        ...
        let elementData = res.detail;
        if (customElementManifestData[elementData.tagName]) {
            // If custom element manifest contains data about this element, merge it with the select data
            elementData = { ...elementData, ...customElementManifestData[elementData.tagName] };
        }
        nydus.message(CONNECTION_CHANNELS.DEVTOOLS_PANEL_TO_CONTENT, {
            type: MESSAGE_TYPE.SELECT_RESULT,
            data: elementData,

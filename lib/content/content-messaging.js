...
//import { analyzeAllScripts, analyzeAndUpdateElement } from 'analyzer';
...
function onDomMessage(message) {
    ...
    switch (message.data.messageType) {
        case SELECT_RESULT:
            ...
            break;
        case QUERY_RESULT:
            console.log("QUERY RESULT ", message.data);
...
function injectScript() {
    if (true) return;
    // TODO: Fix injecting scripts
    // Maybe follow this https://davidwalsh.name/inject-global-mv3
    const existingScript = document.head.querySelector("script[web-component-devtools-script]");
    if (existingScript) return;

    const crawlerUrl = chrome.runtime.getURL('crawler-inject.js');
    const spotlightUrl = chrome.runtime.getURL('spotlight-border.js');

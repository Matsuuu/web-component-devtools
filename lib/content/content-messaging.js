...
function getCustomElementManifestData() {
    ...
    const customElementsPath = document.querySelector("meta[name='custom-elements-manifest']")?.content;
 ...
 * */
function doAnalyze(eventData) {
    return new Promise(resolve => {
        const onAnalyzeResult = async (/** @type {MessageEvent} */ event) => {
            if (event.data.messageType === ANALYZE_RESULT) {
                // Handle custom element decalration fetch request result
                window.removeEventListener('message', onAnalyzeResult);
                const declarationString = event.data.declarationString;
                eventData.declaration = declarationString;
                await analyzeAndUpdateElement(eventData);
                resolve(eventData);
            }
        };
        // Listen for analyze result
        window.addEventListener('message', onAnalyzeResult);
        // Call DOM actions to get custom element declaration
        window.postMessage(
            { indexInDevTools: eventData.indexInDevTools, messageType: ANALYZE_REQUEST },
...
function handleSelectResult(message) {
    ...
    const events = elementData.events;

...
export function findAllElements(elem) {
    ...
    for (const e of elements) {
        ...
        if (getElementNodeName(e) === 'IFRAME') {
            try {
                // @ts-ignore
                const iframeDocument = e.contentWindow.document;
                const contentLength = iframeDocument.body.innerHTML.length
                const isMainDocument = iframeDocument.body === document.body;
                if (!isMainDocument && contentLength > 0) {
                    // @ts-ignore
...
export function findElementsOnPage() {
    console.log("findElementsOnPage")
    let customElements = findCustomElements();
    console.log("calculateElementDepths")

...
export function highlightElement(index) {
    /** @type {import("./spotlight-border.js/index.js").SpotlightBorder} */
...
export async function consoleAction(eventData) {
    ...
    let returnValue = newFunc.call(context);

    let error = null;
    let errorID = null;
    if (returnValue instanceof Node) {
        error = "Value was of type \"Node\", and could only be passed back as a stringified representation. Click here to print the actual result into the DevTools console."
        errorID = "_ERR_IS_NODE";
        returnValue = buildNodeText(returnValue);
    }
    if (typeof returnValue === 'function') {
        error = "Value was of type \"Function\", and could only be passed back as a stringified representation. Click here to print the actual result into the DevTools console."
        errorID = "_ERR_IS_FUNCTION";
        returnValue = buildFunctionText(returnValue);
    }
    window.postMessage({ messageType: CONSOLE_ACTION_RESULT, returnValue, error, errorID, code }, window.location.origin);
}

...
export function selectElement(eventData) {
    ...
    const viableData = {};
    removeUnsendableObjects(viableData, elementProperties, eventData);
 ...
 * */
function removeUnsendableObjects(returnData, elementProperties, eventData, cache = [], layer = 0) {
    if (layer > 100) {
    ...
    for (const [key, val] of Object.entries(elementProperties)) {
        ...
        if (val instanceof Node || typeof val === 'function' || isValidCacheHit(cache, key, val, eventData)) {
        ...
        if (typeof val === 'object' && val != null) {
            ...
            cache.push(val);
            removeUnsendableObjects(returnData[key], val, eventData[key], cache, layer + 1);
        } else {
            returnData[key] = val;
        }
    }

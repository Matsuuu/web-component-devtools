...
export function selectElement(eventData) {
    ...
    const returnEventData = { detail: parseElementProperties(selectedElement, eventData) };

    removeUnsendableObjects(returnEventData);
    console.log(returnEventData);
    document.dispatchEvent(new CustomEvent(SELECT_RESULT, returnEventData));
}

function removeUnsendableObjects(eventData, layer = 0) {
    if (layer > 6) {
    ...
    for (const [key, val] of Object.entries(eventData)) {
        if (val instanceof Node || typeof val === "function") {
            delete eventData[key];
            continue;
        }
        if (typeof val === 'object' && val != null) {
            removeUnsendableObjects(val, layer + 1);
        }
    }
}

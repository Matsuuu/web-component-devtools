    ...
    switch (updateTarget) {
        ...
        case 'property':
            ...
            if (!updateData.propertyPath || updateData.propertyPath.length <= 0) {
                ...
                const propertyPath = [...updateData.propertyPath];
                while (propertyPath.length > 0) {
                    targetObject = targetObject[propertyPath.shift()];
                }
 ...
 * @param {string} updateTarget Property or Attribute
 */
export function updatePolymerElementValue(updateData, updateTarget) {
    updateGenericComponentValue(updateData, updateTarget);

    if (updateTarget === 'property') {
        const element = getCurrentSelectedElement();
        element.notifyPath(updateData.propertyPath);
    }
}
 ...
 * @param {string} updateTarget Property or Attribute
 */
export function updateAngularElementValue(updateData, updateTarget) {
    updateGenericComponentValue(updateData, updateTarget);

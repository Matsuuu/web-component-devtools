...
function determineAttributes(customElementDeclaration, elementData, element) {
    ...
    for (const attrName of observedAttributes) {
        const existingAttribute = elementData.attributes?.find(att => att.name === attrName);
        if (existingAttribute && existingAttribute.type) continue; // This attribute is already declared

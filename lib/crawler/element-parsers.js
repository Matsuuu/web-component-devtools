 ...
 *  @param {DevToolsElement} [manifestData]
 */
export function parseGenericComponent(customElementDeclaration, element, elementState, manifestData) {
    const attributeData = mapObservedAttributes(customElementDeclaration, element);
    const propertyData = mapPropertiesFromManifest(manifestData, element);
    console.log(propertyData);
    elementState.attributes = attributeData.attributes;
    elementState.attributeValues = attributeData.attributeValues;
    elementState.properties = propertyData.properties;
    elementState.propertyValues = propertyData.propertyValues;
}

function mapPropertiesFromManifest(manifestData, element) {
    const properties = [];
    const propertyValues = {};
    if (!manifestData) return { properties, propertyValues };

    for (const prop of manifestData.properties) {
        properties.push(prop); // We handle the props in the same format so we leave it as is
...
export function parseLitElement(customElementDeclaration, element, elementState) {
    ...
    elementState.properties = { ...elementState.properties, ...properties };
    elementState.propertyValues = { ...elementState.propertyValues, propertyValues };
}
...
${mapPropertiesFromManifest.toString()}
`;

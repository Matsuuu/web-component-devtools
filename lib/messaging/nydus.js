...
export class Nydus {
    ...
    async _createRequiredConnections() {
    ...
    _createConnection(nydusConnection, isBackground) {
        return new Promise(async resolve => {
            ...
            this._addConnectionOnDisconnect(connection, nydusConnection.id.toString(), tabId);
            resolve(connection);
        });
    }
     ...
     */
    _addConnectionOnDisconnect(connection, connectionId, tabId) {
    ...
    _doClientHandshake(nydusConnection, onMessage) {
        chrome.runtime.onConnect.addListener(
            function startHandshake(/** @type chrome.runtime.Port */ connection) {
                ...
                this._addConnectionOnDisconnect(connection, nydusConnection.id.toString(), nydusConnection.tabId);
            }.bind(this),
        );
    }
    ...
    _getConnectionsFlat() {
        let connections = [];
        const tabIds = Object.keys(this.connections);
        for (const tabId of tabIds) {
            const tabConnections = this.connections[tabId];
            const connIds = Object.keys(tabConnections);
            for (const connId of connIds) {
                connections.push(this.connections[tabId][connId]);
            }
        }
        return connections;
    }

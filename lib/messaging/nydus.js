...
export class Nydus {
    ...
    async message(recipient, message) {
        ...
        if (!connPool) {
            console.warn('[WebComponentDevTools]: Message send missed. Tab connection pool not found.', { recipient, message, tabId });
            return;
        }

        const conn = connPool[recipient]?.connection;
        if (!conn) {
            console.warn('[WebComponentDevTools]: Message send missed. Connection not found.', { recipient, message, tabId });
            return;
        }
     ...
     * @param {OnMessageCallback} onMessage
     */
    _doClientHandshake(nydusConnection, onMessage, isBackground) {
